...
$bin/hgFakeAgp ../$out.final.A.fa /dev/stdout | awk '{if(\$5==\"D\") {print \$1\"\\t\"\$2-1\"\\t\"$3}}'| $bin/bedtools getfasta -bed /dev/stdin -fi ../$out.final.A.fa -fo /dev/stdout 2>/dev/null | $bin/seq_n50.pl 
